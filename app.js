...
function migrateOldData() {
    let changed = false;
    if (!Array.isArray(state.entries)) { state.entries = []; return; } // Proteção contra dados corrompidos
...
// --- LÓGICA DE INPUT E BOTÕES (CORRIGIDA) ---
...
function updateSignifierButtons() {
    ...
    // SAFEGUARD: Se os botões não existirem no HTML, aborta.
    if (!btnP || !btnI) return;

    // FIX: Busca SVG *OU* tag I (porque Lucide substitui o I por SVG)
    const iconP = btnP.querySelector('svg') || btnP.querySelector('i');
...
function renderGlobalInput() {
    ...
    // FIX: Recria o botão toggle para garantir que o ícone atualize corretamente com Lucide
    // (Lucide não observa atributos, precisa de novos elementos)
    const toggleBtnContainer = document.getElementById('global-type-toggle');
    
    // Se o elemento existe, atualizamos o HTML interno (hard reset do ícone)
    if (toggleBtnContainer) {
        toggleBtnContainer.innerHTML = `
            <i data-lucide="${config.icon}" class="w-4 h-4 text-black dark:text-white" id="global-type-icon"></i>
            <span class="text-xs font-bold text-black hidden sm:inline-block dark:text-white" id="global-type-label">${T(config.label)}</span>
    ...
    updateSignifierButtons();
    // Renderiza ícones NOVOS gerados
...
function addNewEntry() {
    ...
    const inputEl = document.getElementById('global-entry-input');
...
function handleGlobalKeydown(e) {
    ...
    const modal = document.getElementById('global-input-modal');
    if (e.key === 'Enter' && !e.shiftKey && modal && !modal.classList.contains('hidden')) {
        e.preventDefault();
        const input = document.getElementById('global-entry-input');
        if(input) {
...
function render() {
    ...
    const navSettings = document.getElementById('nav-settings-text');
    if(navSettings) navSettings.textContent = T('nav_settings');

    if (state.activeTab === 'home') {
        if(mainContainer) mainContainer.innerHTML = getHomeHtml();
        if(window.lucide) lucide.createIcons();
    } else if (state.activeTab === 'journal') {
        if(mainContainer) mainContainer.innerHTML = getJournalHtml();
        if(window.lucide) lucide.createIcons();
    } else if (state.activeTab === 'hubs') {
        if(mainContainer) mainContainer.innerHTML = getHubsHtml();
        if(window.lucide) lucide.createIcons();
    } else if (state.activeTab === 'collections') {
        if(mainContainer) mainContainer.innerHTML = getCollectionsHtml();
        if(window.lucide) lucide.createIcons();
    } else if (state.activeTab === 'settings') {
        if(mainContainer) mainContainer.innerHTML = getSettingsHtml();
        if(window.lucide) lucide.createIcons();
    }
}

function renderVisualEntry(entry) {
    const config = ENTRY_TYPES[entry.type] || ENTRY_TYPES['task']; // SAFEGUARD para tipos antigos
    ...
    return `
        <div class="flex items-start gap-3 p-3 bg-white border border-stone-100 hover:border-stone-400 group transition-all dark:bg-stone-900 dark:border-stone-800 dark:hover:border-stone-600 relative">
            ...
            <div class="flex-1 min-w-0" onclick="startEditEntry(${entry.id})">
                <p class="text-sm ${statusClass} leading-relaxed font-medium cursor-pointer">${contentHtml}</p>
            </div>
            <div class="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity absolute right-2 top-2 bg-white dark:bg-stone-900 p-1 shadow-sm border border-stone-100 dark:border-stone-700 rounded z-10">
                ...
                ` : ''}
                <button onclick="deleteEntry(${entry.id})" class="text-stone-400 hover:text-red-600" title="Excluir"><i data-lucide="trash-2" class="w-3 h-3"></i></button>
            </div>
        </div>
    `;
}
...
function renderClassicEntry(entry) {
    const config = ENTRY_TYPES[entry.type] || ENTRY_TYPES['task']; // SAFEGUARD
    const isPriority = entry.isPriority;
    const isInspiration = entry.isInspiration;
    const contentHtml = formatContent(entry.content);
    ...
    return `
        <div class="group flex items-baseline gap-2 py-1 px-1 hover:bg-stone-50 rounded -ml-1 transition-colors cursor-default dark:hover:bg-stone-800 font-mono text-sm">
            <div class="w-8 text-right select-none font-bold text-black dark:text-white flex-shrink-0">
                <button onclick="cycleTaskStatus(${entry.id})" class="hover:text-stone-500 transition-colors">${symbol}</button>
            </div>
            <div class="flex-1 min-w-0 ${entry.status === TASK_STATUS.COMPLETED ? 'line-through text-stone-400' : 'text-stone-800 dark:text-stone-300'}" onclick="startEditEntry(${entry.id})">
                ${contentHtml}
            </div>
    ...
    return `
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="flex gap-2">
                 <button onclick="openGlobalInput()" class="bg-black text-white w-10 h-10 rounded-full flex items-center justify-center hover:bg-stone-800 shadow-lg transition-all dark:bg-white dark:text-black dark:hover:bg-stone-200"><i data-lucide="plus" class="w-5 h-5"></i></button>
            </div>
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-8">
                ...
                <section class="bg-stone-50 p-6 rounded border border-stone-100 dark:bg-stone-850 dark:border-stone-800">
                    ...
                    ${nextEvent 
                        ? `<div class="bg-white p-4 rounded border-l-4 border-black shadow-sm dark:bg-stone-900 dark:border-stone-100"><p class="font-bold text-lg mb-1 dark:text-white">${nextEvent.content}</p><p class="text-sm text-stone-500 dark:text-stone-400">${new Date(nextEvent.targetDate).toLocaleDateString(currentLang)}</p></div>`
    ...
    return `
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex bg-stone-100 p-1 rounded dark:bg-stone-800">
                ${['Hoje', 'Futuro', 'Todos'].map(p => `<button onclick="setJournalPeriod('${p}')" class="px-3 py-1 text-sm rounded transition-all ${state.activeJournalPeriod === p ? 'bg-white shadow text-black font-bold dark:bg-stone-700 dark:text-white' : 'text-stone-500 hover:text-stone-900 dark:text-stone-400'}">${T(PERIOD_MAP[p])}</button>`).join('')}
            </div>
        </div>
        <div class="space-y-2">
            ${filteredEntries.length > 0 ? filteredEntries.map(renderer).join('') : `<div class="text-center py-12 text-stone-400">Nenhum registro encontrado.</div>`}
        </div>
    `;
}

function getHubsHtml() {
    return `<h1 class="text-2xl font-bold mb-6 dark:text-white">${T('nav_hubs')}</h1><div class="grid grid-cols-2 gap-4">${state.hubs.map(hub => `<div onclick="filterByHub(${hub.id})" class="p-6 bg-stone-50 border border-stone-200 rounded hover:border-stone-400 cursor-pointer transition-all dark:bg-stone-800 dark:border-stone-700"><i data-lucide="${hub.icon}" class="w-8 h-8 mb-3 text-stone-700 dark:text-stone-300"></i><h3 class="font-bold text-lg dark:text-white">${hub.name}</h3><p class="text-stone-500 text-sm dark:text-stone-400">${state.entries.filter(e => e.hubId === hub.id).length} itens</p></div>`).join('')}</div>`;
}

function getCollectionsHtml() {
    const tags = extractTags();
    return `<h1 class="text-2xl font-bold mb-6 dark:text-white">${T('nav_collections')}</h1><div class="flex flex-wrap gap-3">${tags.map(tag => `<button onclick="filterByTag('${tag}')" class="px-4 py-2 bg-stone-50 border border-stone-200 rounded-full hover:bg-white hover:border-black transition-all font-mono text-sm dark:bg-stone-800 dark:border-stone-700 dark:text-white dark:hover:border-stone-400">#${tag} <span class="text-stone-400 ml-1 text-xs">(${state.entries.filter(e => e.content.includes('#'+tag)).length})</span></button>`).join('')}</div>`;
}
    ...
    return `
        ...
        <div class="space-y-6 max-w-lg">
            <div class="flex items-center justify-between p-4 border border-stone-200 rounded dark:border-stone-700">
                <div><h3 class="font-bold dark:text-white">Modo de Visualização</h3><p class="text-sm text-stone-500 dark:text-stone-400">Escolha entre moderno ou purista.</p></div>
                <button onclick="toggleViewMode()" class="px-3 py-1 bg-stone-100 rounded font-bold text-sm dark:bg-stone-800 dark:text-white">${state.prefs.viewMode === 'visual' ? 'Visual' : 'Clássico'}</button>
            </div>
            <div class="flex items-center justify-between p-4 border border-stone-200 rounded dark:border-stone-700">
                <div><h3 class="font-bold dark:text-white">Tema</h3><p class="text-sm text-stone-500 dark:text-stone-400">Claro ou Escuro (OLED).</p></div>
                <button onclick="toggleTheme()" class="px-3 py-1 bg-stone-100 rounded font-bold text-sm dark:bg-stone-800 dark:text-white">${state.prefs.theme === 'dark' ? 'Escuro' : 'Claro'}</button>
            </div>
             <div class="flex items-center justify-between p-4 border border-stone-200 rounded dark:border-stone-700">
                <div><h3 class="font-bold dark:text-white">Idioma / Language</h3><p class="text-sm text-stone-500 dark:text-stone-400">Português ou English.</p></div>
                <button onclick="toggleLang()" class="px-3 py-1 bg-stone-100 rounded font-bold text-sm dark:bg-stone-800 dark:text-white">${state.prefs.lang === 'pt-BR' ? '🇧🇷 PT' : '🇺🇸 EN'}</button>
            </div>
            <div class="pt-6 border-t border-stone-200 dark:border-stone-700">
             ...
             <div class="pt-6 text-center"><p class="text-xs text-stone-400">Versão 3.1.2 (SVG Fix)</p></div>
        </div>
    `;
}
...
function getNavHtml() {
    ...
    return items.map(item => `<button onclick="setActiveTab('${item.id}'); toggleMobileNav()" class="w-full flex items-center gap-3 px-3 py-2 text-sm rounded transition-all ${state.activeTab === item.id ? 'bg-stone-100 font-bold text-black dark:bg-stone-800 dark:text-white' : 'text-stone-600 hover:bg-stone-50 dark:text-stone-400 dark:hover:bg-stone-800'}"><i data-lucide="${item.icon}" class="w-4 h-4"></i>${T(item.label)}</button>`).join('');
}
